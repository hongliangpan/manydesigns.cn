...
/**
 * A chart class implemented using the Java 2D APIs. The current version supports bar charts, line charts, pie charts
 * and xy plots (including time series data).
 * <P>
 * JFreeChart coordinates several objects to achieve its aim of being able to draw a chart on a Java 2D graphics device:
 * a list of {@link Title} objects (which often includes the chart's legend), a {@link Plot} and a
 * {@link org.jfree.data.general.Dataset} (the plot in turn manages a domain axis and a range axis).
 * <P>
...
public class JFreeChart implements Drawable, TitleChangeListener, PlotChangeListener, Serializable, Cloneable {
	...
	public static final Font DEFAULT_TITLE_FONT = new Font(JFreeChart.getFont(), Font.BOLD, 18);
	...
	/**
	 * The key for a rendering hint that can suppress the generation of a shadow effect when drawing the chart. The hint
	...
	/**
	 * Creates a new chart based on the supplied plot. The chart will have a legend added automatically, but no title
	 * (although you can easily add one later). <br>
	 * <br>
	...
	/**
	 * Creates a new chart with the given title and plot. A default font ({@link #DEFAULT_TITLE_FONT}) is used for the
	 * title, and the chart will have a legend added automatically. <br>
	 * <br>
	 * Note that the {@link ChartFactory} class contains a range of static methods that will return ready-made charts,
	 * and often this is a more convenient way to create charts than using this constructor.
	 * 
	...
	/**
	 * Sets the rendering hints for the chart. These will be added (using the Graphics2D.addRenderingHints() method)
	 * near the start of the JFreeChart.draw() method.
	 * 
	...
	/**
	 * Sets the padding between the chart border and the chart drawing area, and sends a {@link ChartChangeEvent} to all
	...
	/**
	 * Returns the main chart title. Very often a chart will have just one title, so we make this case simple by
	 * providing accessor methods for the main title. However, multiple titles are supported - see the
	...
	/**
	 * Sets the main title for the chart and sends a {@link ChartChangeEvent} to all registered listeners. If you do not
	 * want a title for the chart, set it to <code>null</code>. If you want more than one title on a chart, use the
	 * {@link #addSubtitle(Title)} method.
	 * 
	...
	/**
	 * Sets the chart title and sends a {@link ChartChangeEvent} to all registered listeners. This is a convenience
	 * method that ends up calling the {@link #setTitle(TextTitle)} method. If there is an existing title, its text is
	 * updated, otherwise a new title using the default font is added to the chart. If <code>text</code> is
	 * <code>null</code> the chart title is set to <code>null</code>.
	 * 
	...
	/**
	 * Returns the legend for the chart, if there is one. Note that a chart can have more than one legend - this method
	 * returns the first.
	 * 
	...
	/**
	 * Adds a subtitle at a particular position in the subtitle list, and sends a {@link ChartChangeEvent} to all
	 * registered listeners.
	 * 
	...
	/**
	 * Returns the plot for the chart. The plot is a class responsible for coordinating the visual representation of the
	 * data, including the axes (if any).
	 * 
	 * @return The plot.
	 */
	public Plot getPlot() {
	...
	/**
	 * Sets the value in the rendering hints table for {@link RenderingHints#KEY_TEXT_ANTIALIASING} to either
	 * {@link RenderingHints#VALUE_TEXT_ANTIALIAS_ON} or {@link RenderingHints#VALUE_TEXT_ANTIALIAS_OFF}, then sends a
	...
	/**
	 * Sets the value in the rendering hints table for {@link RenderingHints#KEY_TEXT_ANTIALIASING} and sends a
	 * {@link ChartChangeEvent} to all registered listeners.
	 * 
	...
	/**
	 * Sets the paint used to fill the chart background and sends a {@link ChartChangeEvent} to all registered
	 * listeners.
	 * 
	...
	/**
	 * Returns the background image alignment. Alignment constants are defined in the <code>org.jfree.ui.Align</code>
	...
	/**
	 * Sets the background alignment. Alignment options are defined by the {@link org.jfree.ui.Align} class.
	 * 
	...
	/**
	 * Sets the alpha-transparency for the chart's background image. Registered listeners are notified that the chart
	 * has been changed.
	 * 
	...
	/**
	 * Draws the chart on a Java 2D graphics device (such as the screen or a printer). This method is the focus of the
	 * entire JFreeChart library.
	 * 
	 * @param g2 the graphics device.
	 * @param area the area within which the chart should be drawn.
	...
	/**
	 * Draws a title. The title should be drawn at the top, bottom, left or right of the specified area, and the area
	 * should be updated to reflect the amount of space used by the title.
	 * 
	...
	protected EntityCollection drawTitle(Title t, Graphics2D g2, Rectangle2D area, boolean entities) {
		...
		RectangleConstraint constraint = new RectangleConstraint(ww, new Range(0.0, ww), LengthConstraintType.RANGE,
	...
	/**
	 * Handles a 'click' on the chart. JFreeChart is not a UI component, so some other object (for example,
	 * {@link ChartPanel}) needs to capture the click event and pass it onto the JFreeChart object. If you are not using
...
class JFreeChartInfo extends ProjectInfo {
	...
	public JFreeChartInfo() {
		...
		setContributors(Arrays.asList(new Contributor[] { new Contributor("Eric Alexander", "-"),
				...
				new Contributor("Dan Rivett", "d.rivett@ukonline.co.uk"), new Contributor("Scott Sams", "-"),
