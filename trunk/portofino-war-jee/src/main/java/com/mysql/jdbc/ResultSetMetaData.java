 ...
 * @version $Id: ResultSetMetaData.java,v 1.1.2.1 2005/05/13 18:58:38 mmatthews Exp $
...
public class ResultSetMetaData implements java.sql.ResultSetMetaData {
	 ...
	 * @return the Java character encoding name for the given column, or null if no Java character encoding maps to the
	 *         MySQL character set for the given column.
	 ...
	 * JDBC 2.0
	 * <p>
	 * Return the fully qualified name of the Java class whose instances are manufactured if ResultSet.getObject() is
	 * called to retrieve a value from the column. ResultSet.getObject() may return a subClass of the class returned by
	 * this method.
	 * </p>
	 * 
	...
	public String getColumnClassName(int column) throws SQLException {
		...
		return getClassNameForJavaType(f.getSQLType(), f.isUnsigned(), f.getMysqlType(), f.isBinary() || f.isBlob(),
				f.isOpaqueBinary());
	}
	...
	/**
	 * What is a column's table's schema? This relies on us knowing the table name. The JDBC specification allows us to
	 * return "" if this is not applicable.
	 * 
	 * @param column the first column is 1, the second is 2...
	 * @return the Schema
	 * @throws SQLException if a database access error occurs
	...
	/**
	 * Can the column be used in a WHERE clause? Basically for this, I split the functions into two types: recognised
	 * types (which are always useable), and OTHER types (which may or may not be useable). The OTHER types, for now, I
	 * will assume they are useable. We should really query the catalog to see if they are useable.
	 * 
	 * @param column the first column is 1, the second is 2...
	...
	static String getClassNameForJavaType(int javaType, boolean isUnsigned, int mysqlTypeIfKnown,
			boolean isBinaryOrBlob, boolean isOpaqueBinary) {
		switch (javaType) {
		case Types.BIT:
		case Types.BOOLEAN:
	 ...
	 * calling <code>unwrap</code> recursively on the wrapped object. If the receiver is not a wrapper and does not
