...
<%
    ...
    //System.err.println("navigationItems:"+navigationItems+"");
    boolean first = true;
    int level = 0;
    String title = "";
    %>
    ...
    while (!navigationItems.isEmpty()) {
        ...
        if(level >= startingLevel) {
            if (first) {
                first = false;
                %><ul class="nav nav-list"><% //Con classe "afflix" la navbar rimane sempre visibile, ma bisogna dare width assoluta (Responsive)
            }
        ...
        for (NavigationItem current : navigationItems) {
            ...
            if(level >= startingLevel) {
                ...
                } else if  (current.getTitle()!=null && current.getTitle().startsWith("---")) {
            		xb.addAttribute("class","divider");
                }
        ...
        level++;
       // System.err.println("nextNavigationItem:"+nextNavigationItem+"");
    }
    if(!first) {
        %></ul><%
    }
