...
//不同部分
	Params params = Params.getParams(request,"project_matrix");
...
// 不同部分，项目多一些 订单时间，项目经理等
	 String[][] statMenus = {
			{"c_customer_pm","客户负责人"},{"c_customer_name","客户"},
		 	...
		 	,
			{"c_pm","项目经理"},{"c_category","ABC类别"},
	 		{"c_order_time_y","订单时间(年)"},{"c_order_time_q","订单时间(季度)"},
	 		
	 		
	 };
		//{"c_is_template","样板"},{"c_is_template_boutique","精品样板"},
	 		//{"c_is_difficult","难"},{"c_is_good","用的好"},{"c_is_develop","二次开发"},
...
%>
	...
	<stripes:layout-component name="pageBody">
  	<%
  		//不同部分
  	  	String sql ="SELECT p.c_id ,CASE WHEN ( p.c_short_name = NULL OR p.c_short_name = '' ) THEN p.c_name ELSE p.c_short_name END c_name,p.c_name c_full_name,"
  	  	+" p.c_code, p.c_customer_id,  CONCAT(year(c_order_time),'_',QUARTER(c_order_time)) c_order_time_q,c_order_time,year(c_order_time) c_order_time_y, \n" +
  		 "p.c_money, p.c_begin_date, p.c_end_date, p.c_category, \n" +
  		 "p.c_pm, p.c_state c_state_id, s.c_name c_state,"+
  		 "c.c_name c_customer_name, \n" +
  		 "c.c_region1, c.c_region2, c.c_region3, c.c_industry1, c.c_industry2, \n" +
  		 "c.c_pm c_customer_pm, c.c_pmb, c.c_money c_customer_money, \n" +
  		 "c.c_is_abnormal, c.c_warranty, c.c_warranty_end, IF(DATEDIFF(c_warranty_end,NOW())<0,true,false) c_is_expired, \n" +
  		 "c.c_client_aspiration, c.c_is_template, c.c_is_template_boutique, \n" +
  		 "c.c_is_difficult, c.c_is_good, c.c_is_template_conditions, \n" +
  		 ...
  		 "LEFT JOIN t_dict_state s ON s.c_id=p.c_state\n" +
  		 "LEFT JOIN (\n" +
  		 ...
  		 " WHERE {0} "+
  		 "ORDER BY s.c_sort_id, c_is_difficult DESC, c_is_good DESC, c_is_template_boutique DESC, c_is_template DESC ,c_category,year(c_order_time)"
  		 ;
        	  ...
        	  <% for (String row : MatrixUtils.getStatValues(mapList, params.getStatY(),params.getSortType().getY())) { %>
        		<tr>
        			  ...
        			  <td valign="top">
        			  	...
        			  	%>
        			  	<a href="/t_customer/<%=map.get("c_customer_id")%>" data-roggle="tooltip" title="<%=map.get("c_customer_name")%>---<%=map.get("c_full_name")%>" target="_blank" name="cell_id_<%=params.getStatX()%><%=params.getStatY()%>" <%=(isLarge? "style='display:none;'" : "")%>>
