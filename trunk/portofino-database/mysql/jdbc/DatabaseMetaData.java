 ...
 * </p>
 * <p>
 * Many of the methods here return lists of information in ResultSets. You can use the normal ResultSet methods such as
 * getString and getInt to retrieve the data from these ResultSets. If a given form of metadata is not available, these
 * methods show throw a SQLException.
 * </p>
 * <p>
 * Some of these methods take arguments that are String patterns. These methods all have names such as fooPattern.
 * Within a pattern String "%" means match any substring of 0 or more characters and "_" means match any one character.
 * </p>
 * 
 * @author Mark Matthews
 * @version $Id: DatabaseMetaData.java,v 1.27.4.66 2005/05/03 18:40:39 mmatthews Exp $
 */
public class DatabaseMetaData implements java.sql.DatabaseMetaData {
	...
	class LocalAndReferencedColumns {
		...
		LocalAndReferencedColumns(List<String> localColumns, List<String> refColumns, String constName,
		...
		TypeDescriptor(String typeInfo, String nullabilityInfo) throws SQLException {
			...
			if (StringUtils.startsWithIgnoreCase(typeInfo, "enum")) {
				...
				if ((StringUtils.indexOfIgnoreCase(typeInfo, "char") != -1
						...
						typeInfo, "bit") != -1) && typeInfo.indexOf("(") != -1) {
	...
	static {
		...
		String[] allMySQLKeywords = new String[] { "ACCESSIBLE", "ADD", "ALL", "ALTER", "ANALYZE", "AND", "AS", "ASC",
				"ASENSITIVE", "BEFORE", "BETWEEN", "BIGINT", "BINARY", "BLOB", "BOOLEAN", "BOTH", "BY", "CALL",
				...
				"ELSE", "ELSEIF", "ENCLOSED", "ESCAPED", "EXISTS", "EXIT", "EXPANSION", "EXPLAIN", "FALSE", "FETCH",
				...
				"INTEGER", "INTERVAL", "INTO", "IS", "ITERATE", "JOIN", "KEY", "KEYS", "KILL", "LEADING", "LEAVE",
				...
				"MIDDLEINT", "MINUTE_MICROSECOND", "MINUTE_SECOND", "MOD", "MODE", "MODIFIES", "NATURAL", "NOT",
				"NO_WRITE_TO_BINLOG", "NULL", "NUMERIC", "ON", "OPTIMIZE", "OPTION", "OPTIONALLY", "OR", "ORDER",
				"OUT", "OUTER", "OUTFILE", "QUERY", "PRECISION", "PRIMARY", "PROCEDURE", "PURGE", "RANGE", "READ",
				...
				"SECOND_MICROSECOND", "SELECT", "SENSITIVE", "SEPARATOR", "SET", "SHOW", "SMALLINT", "SPATIAL",
				...
				// adding for 5.6:
				"ALGORITHM", "INPLACE", "COPY", "NONE", "SHARED", "EXCLUSIVE", "DATA", "DIRECTORY", "DISCARD",
				"TABLESPACE", "FLUSH", "TABLES", "EXPORT", "IMPORT", "EXCHANGE" };
	...
	private java.sql.ResultSet buildResultSet(com.mysql.jdbc.Field[] fields, java.util.ArrayList<ResultSetRow> rows)
			throws SQLException {
		return buildResultSet(fields, rows, this.conn);
	}

	static java.sql.ResultSet buildResultSet(com.mysql.jdbc.Field[] fields, java.util.ArrayList<ResultSetRow> rows,
	...
	protected void convertToJdbcFunctionList(String catalog, ResultSet proceduresRs, boolean needsClientFiltering,
			String db, List<ComparableWrapper<String, ResultSetRow>> procedureRows, int nameIndex, Field[] fields)
			throws SQLException {
	...
	/**
	 * Getter to JDBC4 DatabaseMetaData.functionNoTable constant. This method must be overridden by JDBC4 subclasses.
	...
	protected void convertToJdbcProcedureList(boolean fromSelect, String catalog, ResultSet proceduresRs,
			boolean needsClientFiltering, String db, List<ComparableWrapper<String, ResultSetRow>> procedureRows,
			int nameIndex) throws SQLException {
		while (proceduresRs.next()) {
			boolean shouldAdd = true;
	 ...
	 * @param forGetFunctionColumns Indicates whether the column belong to a function. This argument is required for
	 *            JDBC4, in which case this method must be overridden to provide the correct functionality.
	...
	/**
	 * JDBC 2.0 Determine whether or not a visible row delete can be detected by calling ResultSet.rowDeleted(). If
	 ...
	 */
	public List<ResultSetRow> extractForeignKeyForTable(ArrayList<ResultSetRow> rows, java.sql.ResultSet rs,
	...
	/**
	 * Creates a result set similar enough to 'SHOW TABLE STATUS' to allow the same code to work on extracting the
	 * foreign key data
	 * 
									...
									if (type.indexOf("enum") != -1) {
										...
										if (type.indexOf(",") != -1) {
											size = Integer.parseInt(type.substring(type.indexOf("(") + 1,
													type.indexOf(",")));
											decimals = Integer.parseInt(type.substring(type.indexOf(",") + 1,
													type.indexOf(")")));
										} else {
											size = Integer.parseInt(type.substring(type.indexOf("(") + 1,
													type.indexOf(")")));
	...
	/*
	 * * Each row in the ResultSet is a parameter desription or column description with the following fields: <OL> <li>
	 * <B>PROCEDURE_CAT</B> String => procedure catalog (may be null) </li> <li> <B>PROCEDURE_SCHEM</B> String =>
	 * procedure schema (may be null) </li> <li> <B>PROCEDURE_NAME</B> String => procedure name </li> <li>
	 * <B>COLUMN_NAME</B> String => column/parameter name </li> <li> <B>COLUMN_TYPE</B> Short => kind of
	 * column/parameter: <UL> <li> procedureColumnUnknown - nobody knows </li> <li> procedureColumnIn - IN parameter
	 * </li> <li> procedureColumnInOut - INOUT parameter </li> <li> procedureColumnOut - OUT parameter </li> <li>
	 * procedureColumnReturn - procedure return value </li> <li> procedureColumnResult - result column in ResultSet
	 * </li> </ul> </li> <li> <B>DATA_TYPE</B> short => SQL type from java.sql.Types </li> <li> <B>TYPE_NAME</B> String
	 * => SQL type name </li> <li> <B>PRECISION</B> int => precision </li> <li> <B>LENGTH</B> int => length in bytes of
	 * data </li> <li> <B>SCALE</B> short => scale </li> <li> <B>RADIX</B> short => radix </li> <li> <B>NULLABLE</B>
	 * short => can it contain NULL? <UL> <li> procedureNoNulls - does not allow NULL values </li> <li>
	 * procedureNullable - allows NULL values </li> <li> procedureNullableUnknown - nullability unknown </li> </ul>
	 * </li> <li> <B>REMARKS</B> String => comment describing parameter/column </li> </ol> </p> <P> <B>Note:</B> Some
	 * databases may not return the column descriptions for a procedure. Additional columns beyond REMARKS can be
	 * defined by the database. </p> @param catalog a catalog name; "" retrieves those without a catalog @param
	 * schemaPattern a schema name pattern; "" retrieves those without a schema @param procedureNamePattern a procedure
	 * name pattern @param columnNamePattern a column name pattern @return ResultSet each row is a stored procedure
	...
	private void getCallStmtParameterTypes(String catalog, String procName, ProcedureType procType,
			String parameterNamePattern, List<ResultSetRow> resultRows, boolean forGetFunctionColumns)
			throws SQLException {
		...
		try {
			...
			if (this.conn.lowerCaseTableNames() && catalog != null && catalog.length() != 0 && oldCatalog != null
					&& oldCatalog.length() != 0) {
			...
			if (!" ".equals(quoteChar)) {
				dotIndex = StringUtils.indexOfIgnoreCaseRespectQuotes(0, procName, ".", quoteChar.charAt(0),
			...
			if (paramRetrievalRs.next()) {
				...
				if (procedureDef != null && procedureDef.length() != 0) {
					...
					int openParenIndex = StringUtils.indexOfIgnoreCaseRespectQuotes(0, procedureDef, "(",
							quoteChar.charAt(0), !this.conn.isNoBackslashEscapesSet());
					int endOfParamDeclarationIndex = 0;

					endOfParamDeclarationIndex = endPositionOfParameterDeclaration(openParenIndex, procedureDef,
							quoteChar);
	...
	private int endPositionOfParameterDeclaration(int beginIndex, String procedureDef, String quoteChar)
		...
		while (parenDepth > 0 && currentPos < procedureDef.length()) {
			int closedParenIndex = StringUtils.indexOfIgnoreCaseRespectQuotes(currentPos, procedureDef, ")",
					quoteChar.charAt(0), !this.conn.isNoBackslashEscapesSet());
	...
	/**
	 * Finds the end of the RETURNS clause for SQL Functions by using any of the keywords allowed after the RETURNS
	 * clause, or a label.
	 * 
	 ...
	 * @param positionOfReturnKeyword the position of "RETRUNS" in the definition
	...
	private int findEndOfReturnsClause(String procedureDefn, String quoteChar, int positionOfReturnKeyword)
		...
		// Label?
		endOfReturn = StringUtils.indexOfIgnoreCaseRespectQuotes(startLookingAt, procedureDefn, ":",
				quoteChar.charAt(0), !this.conn.isNoBackslashEscapesSet());
		...
		if (catalogSpec != null) {
			if (!catalogSpec.equals("")) {
				if (conn.getPedantic()) {
					...
					allCatalogsIter = new SingleStringIterator(StringUtils.unQuoteIdentifier(catalogSpec,
	 ...
	 * ordered by TABLE_SCHEM, TABLE_NAME and ORDINAL_POSITION.
	 * </p>
	 * <P>
	 * Each column description has the following columns:
	 * <OL>
	 ...
	 * <li><B>TYPE_NAME</B> String => Data source dependent type name</li>
	 * <li><B>COLUMN_SIZE</B> int => column size. For char or date types this is the maximum number of characters, for
	 * numeric or decimal types this is precision.</li>
	 ...
	 * <li><B>IS_NULLABLE</B> String => "NO" means column definitely does not allow NULL values; "YES" means the column
	 * might allow NULL values. An empty string means nobody knows.</li>
	 * </ol>
	 * </p>
	 * 
	 * @param catalog a catalog name; "" retrieves those without a catalog
					...
					for (String tableName : tableNameList) {
						...
						try {
							...
							while (results.next()) {
								...
								TypeDescriptor typeDesc = new TypeDescriptor(results.getString("Type"),
										results.getString("Null"));
								...
								if (typeDesc.columnSize == null) {
									...
									if (collation != null
											&& ("TEXT".equals(typeDesc.typeName)
													|| "TINYTEXT".equals(typeDesc.typeName) || "MEDIUMTEXT"
	...
	/**
	 * Get a description of the foreign key columns in the foreign key table that reference the primary key columns of
	 * the primary key table (describe how one table imports another's key.) This should normally return a single
	 * foreign key/primary key pair (most tables only import a foreign key from a table once.) They are ordered by
										...
										while (commentTokens.hasMoreTokens()) {
											...
											while (referencingColumns.hasNext()) {
												String referencingColumn = StringUtils.unQuoteIdentifier(
														referencingColumns.next(), conn.useAnsiQuotedIdentifiers());
	...
	/**
	 * Get a description of a foreign key columns that reference a table's primary key columns (the foreign keys
	 * exported by a table). They are ordered by FKTABLE_CAT, FKTABLE_SCHEM, FKTABLE_NAME, and KEY_SEQ.
	 * <P>
	 * Each foreign key column description has the following columns:
	 * <OL>
	 * <li><B>PKTABLE_CAT</B> String => primary key table catalog (may be null)</li>
								...
								if ((tableType != null)
									...
									if (comment != null) {
										...
										if (commentTokens.hasMoreTokens()) {
											...
											while (commentTokens.hasMoreTokens()) {
												String keys = commentTokens.nextToken();
												getExportKeyResults(catalogStr, tableNameWithCase, keys, rows,
	...
	/**
	 * Adds to the tuples list the exported keys of exportingTable based on the keysComment from the 'show table status'
	 * sql command. KeysComment is that part of the comment field that follows the "InnoDB free ...;" prefix.
	 * 
	 ...
	 * @throws SQLException if a database access error occurs
	 */
	protected void getExportKeyResults(String catalog, String exportingTable, String keysComment,
			List<ResultSetRow> tuples, String fkTableName) throws SQLException {
	...
	/**
	 * Returns the DELETE and UPDATE foreign key actions from the given 'SHOW TABLE STATUS' string, with the DELETE
	 * action being the first item in the array, and the UPDATE action being the second.
	 * 
	...
	/**
	 * Get a description of the primary key columns that are referenced by a table's foreign key columns (the primary
	 * keys imported by a table). They are ordered by PKTABLE_CAT, PKTABLE_SCHEM, PKTABLE_NAME, and KEY_SEQ.
	 * <P>
	...
	/**
	 * Populates the tuples list with the imported keys of importingTable based on the keysComment from the 'show table
	 * status' sql command. KeysComment is that part of the comment field that follows the "InnoDB free ...;" prefix.
	 * 
	 * @param catalog the database to use
	 ...
	 * @throws SQLException if a database access error occurs
	 */
	protected void getImportKeyResults(String catalog, String importingTable, String keysComment,
			List<ResultSetRow> tuples) throws SQLException {
		getResultsImpl(catalog, importingTable, keysComment, tuples, null, false);
	}

	/**
	 * Get a description of a table's indices and statistics. They are ordered by NON_UNIQUE, TYPE, INDEX_NAME, and
	 * ORDINAL_POSITION.
	 * <P>
	 ...
	 * <UL>
	 * <li>tableIndexStatistic - this identifies table statistics that are returned in conjuction with a table's index
	 * descriptions</li>
	 ...
	 * <li><B>ASC_OR_DESC</B> String => column sort sequence, "A" => ascending, "D" => descending, may be null if sort
	 * sequence is not supported; null when TYPE is tableIndexStatistic</li>
	 * <li><B>CARDINALITY</B> int => When TYPE is tableIndexStatisic then this is the number of rows in the table;
	 * otherwise it is the number of unique values in the index.</li>
	 * <li><B>PAGES</B> int => When TYPE is tableIndexStatisic then this is the number of pages used for the table,
	 * otherwise it is the number of pages used for the current index.</li>
	 ...
	 * @param schema a schema name pattern; "" retrieves those without a schema
	 * @param table a table name
	 * @param unique when true, return only indices for unique values; when false, return indices regardless of whether
	 *            unique or not
	...
	public java.sql.ResultSet getIndexInfo(String catalog, String schema, final String table, final boolean unique,
		...
		/*
		 * MySQL stores index information in the following fields: Table Non_unique Key_name Seq_in_index Column_name
				...
				void forEach(String catalogStr) throws SQLException {
					...
					try {
						...
						while (results != null && results.next()) {
							...
							IndexMetaDataKey indexInfoKey = new IndexMetaDataKey(!indexIsUnique, indexType, results
									.getString("Key_name").toLowerCase(), results.getShort("Seq_in_index"));
	 ...
	 * PROCEDURE_SCHEM and PROCEDURE_NAME. Within this, the return value, if any, is first. Next are the parameter
	 * descriptions in call order. The column descriptions follow in column number order.
	 * </p>
	 * <P>
	 ...
	 * <li><b>CHAR_OCTET_LENGTH</b> int => the maximum length of binary and character based columns. For any other
	 * datatype the returned value is a NULL</li>
	 * <li><b>ORDINAL_POSITION</b> int => the ordinal position, starting from 1. A value of 0 is returned if this row
	 * describes the procedure's return value.</li>
	...
	protected java.sql.ResultSet getProcedureOrFunctionColumns(Field[] fields, String catalog, String schemaPattern,
		...
		if (supportsStoredProcedures()) {
			try {
				...
				if ((procedureOrFunctionNamePattern != null) && (!procedureOrFunctionNamePattern.equals("%"))) {
					tmpProcedureOrFunctionNamePattern = StringUtils
							.sanitizeProcOrFuncName(procedureOrFunctionNamePattern);
				}
				...
				procsAndOrFuncsRs = getProceduresAndOrFunctions(createFieldMetadataForGetProcedures(), catalog,
						schemaPattern, tmpProcedureOrFunctionNamePattern, returnProcedures, returnFunctions);
				...
				while (procsAndOrFuncsRs.next()) {
					...
					procsOrFuncsToExtractList
							.add(new ComparableWrapper<String, ProcedureType>(tmpstrCatNameRs + "." + tmpstrPNameRs,
		...
		for (ComparableWrapper<String, ProcedureType> procOrFunc : procsOrFuncsToExtractList) {
			...
			if (!" ".equals(this.quotedId)) {
				idx = StringUtils.indexOfIgnoreCaseRespectQuotes(0, procName, ".", this.quotedId.charAt(0),
						!this.conn.isNoBackslashEscapesSet());
			} else {
			...
			getCallStmtParameterTypes(catalog, procNameToCall, procType, columnNamePattern, resultRows,
			...
			new IterateBlock<String>(getCatalogIterator(catalog)) {
				void forEach(String catalogStr) throws SQLException {
					...
					try {
						...
						if (returnFunctions) {
							convertToJdbcFunctionList(db, proceduresRs, needsClientFiltering, db, procedureRowsToSort,
									nameIndex, fields);
						}
	...
	private void getResultsImpl(String catalog, String table, String keysComment, List<ResultSetRow> tuples,
		...
		while (localColumnNames.hasNext()) {
			byte[][] tuple = new byte[14][];
			String lColumnName = StringUtils.unQuoteIdentifier(localColumnNames.next(),
					this.conn.useAnsiQuotedIdentifiers());
			String rColumnName = StringUtils.unQuoteIdentifier(referColumnNames.next(),
					this.conn.useAnsiQuotedIdentifiers());
	 ...
	 * <P>
	 * Only privileges matching the schema and table name criteria are returned. They are ordered by TABLE_SCHEM,
	 ...
	 * <P>
	 * Only table descriptions matching the catalog, schema, table name and type criteria are returned. They are ordered
	 ...
	 * <li><B>TABLE_SCHEM</B> String => table schema (may be null)</li>
	 * <li><B>TABLE_NAME</B> String => table name</li>
	 * <li><B>TABLE_TYPE</B> String => table type. Typical types are "TABLE", "VIEW", "SYSTEM TABLE",
	 * "GLOBAL TEMPORARY", "LOCAL TEMPORARY", "ALIAS", "SYNONYM".</li>
	...
	public java.sql.ResultSet getTables(String catalog, String schemaPattern, String tableNamePattern,
		...
		List<String> parseList = StringUtils.splitDBdotName(tableNamePattern, tmpCat, quotedId,
							...
							if (hasTableTypes) {
								...
								case TABLE:
									...
									if (operatingOnSystemDB && shouldReportSystemTables) {
										...
										tablesKey = new TableMetaDataKey(TableType.SYSTEM_TABLE.getName(), catalogStr,
												null, results.getString(1));
										reportTable = true;

									} else if (!operatingOnSystemDB && shouldReportTables) {
										row[3] = TableType.TABLE.asBytes();
										tablesKey = new TableMetaDataKey(TableType.TABLE.getName(), catalogStr, null,
												results.getString(1));
										reportTable = true;
									}
								...
								case VIEW:
									if (shouldReportViews) {
										...
										sortedRows
												.put(new TableMetaDataKey(TableType.VIEW.getName(), catalogStr, null,
														results.getString(1)), new ByteArrayRow(row,
														getExceptionInterceptor()));
									}
									break;
	...
	protected Field[] createTablesFields() {
		...
		fields[4] = new Field("", "REMARKS", java.sql.Types.VARCHAR, 300);
	...
	/**
	 * Get a description of all the standard SQL types supported by this database. They are ordered by DATA_TYPE and
	...
	public java.sql.ResultSet getTypeInfo() throws SQLException {
		...
		/*
		 * The following are ordered by java.sql.Types, and then by how closely the MySQL type matches the JDBC Type
		 * (per spec)
		 */
		/*
		...
		/*
		 * MySQL Type: VARBINARY (sliently converted to VARCHAR(M) BINARY) JDBC Type: VARBINARY
		 */
		rowVal = new byte[18][];
		...
		/*
		 * MySQL Type: BINARY (silently converted to CHAR(M) BINARY) JDBC Type: BINARY
		 */
		rowVal = new byte[18][];
		rowVal[0] = s2b("BINARY");
		...
		/*
		 * MySQL Type: FLOAT JDBC Type: REAL (this is the SINGLE PERCISION floating point type)
		 */
		rowVal = new byte[18][];
		rowVal[0] = s2b("FLOAT");
	...
	/**
	 * JDBC 2.0 Get a description of the user-defined types defined in a particular schema. Schema specific UDTs may
	 * have type JAVA_OBJECT, STRUCT, or DISTINCT.
	 * <P>
	 * Only types matching the catalog, schema, type name and type criteria are returned. They are ordered by DATA_TYPE,
	 * TYPE_SCHEM and TYPE_NAME. The type name parameter may be a fully qualified name. In this case, the catalog and
	 ...
	 * </p>
	 * 
	 * @param catalog a catalog name; "" retrieves those without a catalog; null means drop catalog name from the
	 *            selection criteria
	 * @param schemaPattern a schema name pattern; "" retrieves those without a schema
	...
	/**
	 * Get a description of a table's columns that are automatically updated when any value in a row is updated. They
	 * are unordered.
	 * <P>
	 * Each column description has the following columns:
	 * <OL>
						...
						if (whereBuf.length() > 0 || rsFields.size() > 0) {
							...
							while (results.next()) {
								if (with_where || rsFields.contains(results.getString("Field"))) {
									...
									// COLUMN_SIZE
									rowVal[4] = typeDesc.columnSize == null ? null
											: s2b(typeDesc.columnSize.toString());
									// BUFFER_LENGTH
									rowVal[5] = s2b(Integer.toString(typeDesc.bufferLength));
									// DECIMAL_DIGITS
									rowVal[6] = typeDesc.decimalDigits == null ? null : s2b(typeDesc.decimalDigits
											.toString());
									// PSEUDO_COLUMN
	...
	protected LocalAndReferencedColumns parseTableStatusIntoLocalAndReferencedColumns(String keysComment)
		...
		String constraintName = StringUtils.unQuoteIdentifier(keysComment.substring(0, indexOfOpenParenLocalColumns)
				.trim(), this.conn.useAnsiQuotedIdentifiers());
		...
		int indexOfRefer = StringUtils.indexOfIgnoreCaseRespectQuotes(0, keysCommentTrimmed, "REFER ",
				this.quotedId.charAt(0), true);
		...
		String referCatalog = StringUtils.unQuoteIdentifier(referCatalogTableString.substring(0, indexOfSlash),
				this.conn.useAnsiQuotedIdentifiers());
		String referTable = StringUtils.unQuoteIdentifier(referCatalogTableString.substring(indexOfSlash + 1).trim(),
				this.conn.useAnsiQuotedIdentifiers());
		...
		List<String> referColumnsList = StringUtils.split(referColumnNamesString, columnsDelimitter, this.quotedId,
				this.quotedId, false);
		List<String> localColumnsList = StringUtils.split(localColumnNamesString, columnsDelimitter, this.quotedId,
				this.quotedId, false);
	...
	/**
	 * Converts the given string to bytes, using the connection's character encoding, or if not available, the JVM
	 * default encoding.
	 * 
	 * @param s DOCUMENT ME!
	 * @return DOCUMENT ME!
	...
	/**
	 * Does the database store mixed case quoted SQL identifiers in lower case? A JDBC compliant driver will always
	 * return false.
	 * 
	 * @return true if so
	 * @throws SQLException DOCUMENT ME!
	...
	/**
	 * Does the database store mixed case quoted SQL identifiers in mixed case? A JDBC compliant driver will always
	 * return false.
	 * 
	 * @return true if so
	 * @throws SQLException DOCUMENT ME!
	...
	/**
	 * Does the database store mixed case quoted SQL identifiers in upper case? A JDBC compliant driver will always
	 * return true.
	 * 
	 * @return true if so
	 * @throws SQLException DOCUMENT ME!
	 ...
	 * as required. A JDBC compliant driver always returns true.
	 * </p>
	 * 
	 * @return true if so
	 * @throws SQLException DOCUMENT ME!
	...
	/**
	 * If table correlation names are supported, are they restricted to be different from the names of the tables? A
	...
	protected Field[] createFunctionColumnsFields() {
		Field[] fields = { new Field("", "FUNCTION_CAT", Types.VARCHAR, 512),
				...
				new Field("", "COLUMN_TYPE", Types.VARCHAR, 64), new Field("", "DATA_TYPE", Types.SMALLINT, 6),
				new Field("", "TYPE_NAME", Types.VARCHAR, 64), new Field("", "PRECISION", Types.INTEGER, 12),
				new Field("", "LENGTH", Types.INTEGER, 12), new Field("", "SCALE", Types.SMALLINT, 12),
				new Field("", "RADIX", Types.SMALLINT, 6), new Field("", "NULLABLE", Types.SMALLINT, 6),
				new Field("", "REMARKS", Types.VARCHAR, 512), new Field("", "CHAR_OCTET_LENGTH", Types.INTEGER, 32),
				new Field("", "ORDINAL_POSITION", Types.INTEGER, 32), new Field("", "IS_NULLABLE", Types.VARCHAR, 12),
	 ...
	 * ordered by <code>FUNCTION_CAT</code>, <code>FUNCTION_SCHEM</code>, <code>FUNCTION_NAME</code> and
	 ...
	 * </UL>
	 * <LI><B>SPECIFIC_NAME</B> String => the name which uniquely identifies this function within its schema. This is a
	 * user specified, or DBMS generated, name that may be different then the <code>FUNCTION_NAME</code> for example
	 ...
	 *            retrieves those without a schema; <code>null</code> means that the schema name should not be used to
	 *            narrow the search
	...
	public java.sql.ResultSet getPseudoColumns(String catalog, String schemaPattern, String tableNamePattern,
		...
		Field[] fields = { new Field("", "TABLE_CAT", Types.VARCHAR, 512),
				new Field("", "TABLE_SCHEM", Types.VARCHAR, 512), new Field("", "TABLE_NAME", Types.VARCHAR, 512),
				new Field("", "COLUMN_NAME", Types.VARCHAR, 512), new Field("", "DATA_TYPE", Types.INTEGER, 12),
				new Field("", "COLUMN_SIZE", Types.INTEGER, 12), new Field("", "DECIMAL_DIGITS", Types.INTEGER, 12),
				new Field("", "NUM_PREC_RADIX", Types.INTEGER, 12), new Field("", "COLUMN_USAGE", Types.VARCHAR, 512),
				new Field("", "REMARKS", Types.VARCHAR, 512), new Field("", "CHAR_OCTET_LENGTH", Types.INTEGER, 12),
