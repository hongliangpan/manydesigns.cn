	...
	public void init() {
		...
		if (database != null && query != null) {
				...
				if (actualRelationTable != null) {
					if (manySelectionProvider != null) {
						...
						if (manySelectionProvider.getActualSelectionProvider() != null) {
							ModelSelectionProvider actualSelectionProvider = manySelectionProvider
									.getActualSelectionProvider();
							String manyDatabaseName = actualSelectionProvider.getToDatabase();
							actualManyDatabase = DatabaseLogic.findDatabaseByName(persistence.getModel(),
									manyDatabaseName);
			...
			if (StringUtils.isBlank(oneExpression)) {
				...
				try {
					actualOnePropertyName = getOneSelectionProvider().getActualSelectionProvider().getReferences()
							.get(0).getActualFromColumn().getActualPropertyName();
				} catch (Throwable t) {
