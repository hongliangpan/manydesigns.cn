...
CREATE TABLE IF NOT EXISTS tmp_tree_node
(
...
DELIMITER $$
DROP PROCEDURE  IF EXISTS updateDeptInfo $$
 CREATE PROCEDURE updateDeptInfo (IN rootId INT)
 BEGIN
...
c_path_name varchar(500) DEFAULT NULL COMMENT '全路径名称'
  ...
  CALL generateChildDepts(rootId,2);
  UPDATE t_sys_dept d,tmp_tree_node tmp  SET d.c_level=tmp.c_level,d.c_path = tmp.c_path,d.c_full_path_name=tmp.c_path_name WHERE d.c_id =tmp.c_id;
  COMMIT;
 END;
 $$
DROP PROCEDURE  IF EXISTS generateChildDepts $$
 CREATE PROCEDURE generateChildDepts(IN rootId INT,IN nDepth INT)
 BEGIN
  ...
  SELECT c_id FROM t_sys_dept WHERE c_parent_id=rootId;
  ...
  INSERT INTO tmp_tree_node 
	SELECT NULL, t.c_id, 
	  ...
	  FROM t_sys_dept t 
	  INNER JOIN t_sys_dept p  ON t.c_parent_id = p.c_id
  ...
  WHILE done=0 DO
          CALL generateChildDepts(v_id,nDepth+1);
          FETCH cur1 INTO v_id;
  END WHILE;
...
AFTER INSERT ON t_sys_dept
FOR EACH ROW
BEGIN
	call updateDeptInfo(1);
END$$

DROP TRIGGER IF EXISTS tri_dept_update$$
CREATE TRIGGER tri_dept_update
AFTER UPDATE ON t_sys_dept
FOR EACH ROW
BEGIN
   call updateDeptInfo(1);
END$$*/
