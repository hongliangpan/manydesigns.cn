...
public class SelectField extends AbstractField {
	...
	public static SelectionProvider createValuesSelectionProvider(PropertyAccessor accessor, Object[] values,
			String[] labels) {
	...
	public void valueToXhtmlEdit(XhtmlBuffer xb) {
		...
		/*
		 * TODO: precedente versione if (selectionModel.getSelectionProvider().isAutocomplete()) { } else { }
		 */
	}
	...
	public String composeCreateNewJs() {
		...
		sb.append(MessageFormat.format("function popupCloseCallback_{0}(val) '{'"
				+ "if(val) updateSelectOptions(''{1}'', {2}, ''jsonSelectFieldOptions''",
				StringEscapeUtils.escapeJavaScript(id), StringEscapeUtils.escapeJavaScript(selectionModel.getName()),
				selectionModelIndex));
		appendIds(sb);
		sb.append(");}");
		return sb.toString();
	...
	public String composeDropDownJs() {
		StringBuilder sb = new StringBuilder();
		sb.append(MessageFormat.format("$(''#{0}'').change(" + "function() '{'"
	...
	public String composeAutocompleteJs() {
		StringBuilder sb = new StringBuilder();
		sb.append(MessageFormat.format("setupAutocomplete(''#{0}'', ''{1}'', {2}, ''jsonAutocompleteOptions''",
	...
	public void valueToXhtmlEditRadio(XhtmlBuffer xb) {
		...
		if (!required) {
			...
			boolean checked = (value == null);
			writeRadioWithLabel(xb, radioId, getText("elements.field.select.none"), "", checked);
			counter++;
		}
